...
describe('Monitoring group in the admin sidebar', () => {
  it('shows all three monitoring sections to super admins', () => {
    render(<App />)
    expect(screen.getByRole('button', { name: 'الزوار' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'نشاط تسجيل الدخول' })).toBeInTheDocument()
    expect(screen.getByRole('button', { name: 'التنبيهات الأمنية' })).toBeInTheDocument()
  })
  ...
  it('hides monitoring from operators (existing role scope unchanged)', () => {
    ...
    render(<App />)
    expect(screen.queryByRole('button', { name: 'الزوار' })).toBeNull()
    expect(screen.queryByRole('button', { name: 'نشاط تسجيل الدخول' })).toBeNull()
    expect(screen.queryByRole('button', { name: 'التنبيهات الأمنية' })).toBeNull()
  ...
  it('denies the monitoring routes to operators even via direct hash', async () => {
    ...
    expect(screen.queryByRole('heading', { name: /نشاط الزوار/ })).toBeNull()
    expect(screen.getByRole('heading', { name: 'الدخول مقيّد' })).toBeInTheDocument()
  })
})
...
describe('Monitoring routes', () => {
  it('renders the Visitor Activity section at #/visitors', () => {
    goTo('/admin#/visitors')
    render(<App />)
    expect(screen.getByRole('heading', { name: 'نشاط الزوار' })).toBeInTheDocument()
    // Both the topbar subtitle and the page description introduce the section.
    expect(screen.getAllByText(/حركة الزوار بشكل تقريبي/).length).toBeGreaterThanOrEqual(1)
  })

  it('renders the Authentication Activity section at #/auth', () => {
    goTo('/admin#/auth')
    render(<App />)
    expect(screen.getByRole('heading', { name: 'نشاط تسجيل الدخول' })).toBeInTheDocument()
  })

  it('renders the Security Alerts section at #/alerts', () => {
    goTo('/admin#/alerts')
    render(<App />)
    expect(screen.getByRole('heading', { name: 'التنبيهات الأمنية' })).toBeInTheDocument()
    expect(screen.getByText(/حدود الاكتشاف/)).toBeInTheDocument()
  })
  ...
  it('switches sections through the sidebar without a page reload', async () => {
    render(<App />)
    await act(async () => {
      screen.getByRole('button', { name: 'نشاط تسجيل الدخول' }).click()
