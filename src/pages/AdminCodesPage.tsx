...
export function AdminCodesPage({ admin }: { admin: AdminProfile }) {
  ...
  const loadCodes = useCallback(async () => {
    ...
    try {
      ...
      setLoadError(friendlyControlError(cause, 'Admin codes could not be loaded.'))
    } finally {
      setLoading(false)
    }
  }, [])
    ...
    try {
      ...
      if (confirming.action === 'revoke') {
        ...
        try {
          ...
          auditError = friendlyControlError(cause, 'The code was deleted, but the audit event could not be recorded.')
        }
        success('Admin code deleted.')
      }
      if (auditError) error(`Admin code action completed, but audit logging failed: ${auditError}`)
      setConfirming(null)
      await loadCodes()
    } catch (cause) {
      error(friendlyControlError(cause, 'The admin code action could not be completed.'))
    }
  }
  ...
  async function toggleCode(code: AdminCodeSummary) {
    try {
      ...
      try {
        ...
        auditError = friendlyControlError(cause, 'The code status changed, but the audit event could not be recorded.')
      }
      success(code.active ? 'Admin code deactivated.' : 'Admin code activated.')
      if (auditError) error(`Admin code status changed, but audit logging failed: ${auditError}`)
      await loadCodes()
    } catch (cause) {
      error(friendlyControlError(cause, 'The admin code status could not be updated.'))
    }
  }
  ...
  return <>
    ...
    <section className="panel p-0 sm:p-0"><div className="p-4 sm:p-5"><PanelHeading icon={FileKey2} title="Code inventory" description="Hashes are retained for verification; plaintext codes never appear in this table." /></div>{loading ? <div className="px-4 pb-5 sm:px-5"><LoadingRows count={5} /></div> : codes.length === 0 ? <EmptyState icon={FileKey2} title="No access codes yet" description="Create a code when another administrator needs time-bound access." action={<button type="button" className="btn-secondary" onClick={() => setShowCreate(true)}><Plus className="h-4 w-4" /> Create first code</button>} /> : <CodeTable codes={codes} canDelete={admin.role === 'super_admin'} onToggle={toggleCode} onRevoke={(code) => setConfirming({ action: 'revoke', code })} onDelete={(code) => setConfirming({ action: 'delete', code })} />}</section>
...
function CreateCodeDialog({ open, admin, onClose, onCreated, onError }: { open: boolean; admin: AdminProfile; onClose: () => void; onCreated: (created: Awaited<ReturnType<typeof createAdminCode>>) => void; onError: (message: string) => void }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      onCreated(created)
      if (auditError) onError(`Admin code created, but audit logging failed: ${auditError}`)
    } catch (cause) {
      onError(friendlyControlError(cause, 'Admin code could not be created.'))
