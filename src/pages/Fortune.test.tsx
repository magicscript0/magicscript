...
describe('Apple of Fortune game console', () => {
  ...
  it('NEW ROUND creates a validated 50-position round, then REVEAL shows it', () => {
    ...
    // the safe/apple visual, the 32 stored "0" positions render the
    // broken/trap visual — exactly the APP 2 /m11 semantics.
    expect(revealed.filter((cell) => cell.getAttribute('aria-label')?.endsWith('— safe'))).toHaveLength(18)
    expect(revealed.filter((cell) => cell.getAttribute('aria-label')?.endsWith('— bomb'))).toHaveLength(32)
  })
